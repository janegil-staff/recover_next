...
// ── Sober streaks ──────────────────────────────────────────────────────────
// Replaces SubstanceAmountTrend. Shows current streak, longest streak, and
// a heatmap of the last N days. The single most clinically meaningful chart
// for a recovery dashboard — patients hit milestones in days, not weeks.
function SoberStreaks({ records, c, t }) {
  const stats = useMemo(() => {
    ...
    // records are already sorted ascending by date
    ...
    // Capture trailing run (still ongoing)
    if (runLength > 0) {
      streaks.push({
        start: runStart,
        length: runLength,
    ...
    // Current streak: must end on most recent day
  ...
  // Top streaks (max 5) for the bar list
  const topStreaks = useMemo(
    () => [...stats.streaks].sort((a, b) => b.length - a.length).slice(0, 5),
    [stats.streaks],
  );

  // Color thresholds — matches the rest of the app's traffic-light scale
  const streakColor = (n) => {
    if (n >= 30) return "#16A34A"; // green - month+
    if (n >= 14) return "#22C55E"; // light green - 2 weeks+
    if (n >= 7) return "#7AABDB"; // blue - week+
    if (n >= 3) return "#FBBF24"; // amber - 3+ days
    return "#FB923C"; // orange - getting started
  };
...
function SubstanceMixDonut({ records, c, t }) {
  ...
  // Unique days = actual record count. The slice .days values may sum higher
  // when multi-substance days exist, but that's expected for the breakdown.
  const totalUniqueDays = records.length;
  const sliceTotal = data.reduce((s, d) => s + d.days, 0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      {!isEmpty ? (
        ...
        >
          {data.map((d) => {
            // % of all logged days that included this substance (or were sober)
            const pct =
              totalUniqueDays > 0
    ...
    <div style={{ maxWidth: 880, margin: "0 auto", width: "100%" }}>
      ...
      {records.length > 0 && (
        ...
        >
          ...
          {allSubs.length >= 3 && (
            <Card
              title={t.substanceProfile ?? "Substance Profile"}
              subtitle={
                t.daysAndAvgPerSubstance ??
                "Days used & avg amount per substance"
              }
            >
