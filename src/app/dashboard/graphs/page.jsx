  ...
  return (
    <div
      style={{
        ...
        borderRadius: 10,
        padding: "10px 14px",
        boxShadow: "0 4px 16px rgba(45,74,110,0.12)",
      }}
    >
      <div
        style={{ fontSize: 11, fontWeight: 700, color: AD, marginBottom: 6 }}
      >
        {label}
      </div>
      {payload.map((p, i) => (
        ...
        >
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: AD }}>
            {p.value}
          </span>
        </div>
      ))}
  ...
  const moodData = useMemo(
    () =>
      records.map((r) => ({
        ...
        [t.mood ?? "mood"]: r.mood ?? null,
        [t.cravings ?? "cravings"]: r.cravings ?? null,
        [t.wellbeing ?? "wellbeing"]: r.wellbeing ?? null,
        // keep raw keys for internal use
        mood: r.mood ?? null,
  ...
  return (
    <div>
      ...
      >
        ...
        {[7, 30, 90, 365].map((r) => (
          <button
            ...
            onClick={() => setRange(r)}
            style={{
              background: range === r ? A : SU,
              color: range === r ? "#fff" : MU,
              border: `1px solid ${range === r ? A : BO}`,
              borderRadius: 20,
              padding: "5px 14px",
              fontSize: 11,
              fontWeight: 600,
        ...
        <span style={{ fontSize: 11, color: MU, marginLeft: 4 }}>
      ...
      {substanceData.length > 0 && allSubs.length > 0 && (
        <Card
          title={t.substancesByWeek ?? "Substance Use"}
          subtitle={`${t.days ?? "days"} / week`}
        >
          <ResponsiveContainer width="100%" height={240}>
            <BarChart
              data={substanceData}
          ...
          {qScores.map((q) => (
            <div key={q.key}>
              ...
              >
                ...
                {q.score != null ? (
                  <span
                    style={{ fontSize: 12, fontWeight: 700, color: q.color }}
                  >
                    {q.score} / {q.max} ({q.pct}%)
                  </span>
                ) : (
                  <span
                    style={{ fontSize: 11, color: MU, fontStyle: "italic" }}
                  >
      ...
      {moodData.filter((d) => d.mood && d.cravings).length > 3 && (
        <Card
          title={`${t.mood ?? "Mood"} vs ${t.cravings ?? "Cravings"}`}
          subtitle="1–5"
        >
          <ResponsiveContainer width="100%" height={200}>
            <LineChart
              data={moodData.filter((d) => d.mood && d.cravings)}
              margin={{ top: 4, right: 10, left: -20, bottom: 0 }}
            >
              <CartesianGrid
                strokeDasharray="3 3"
