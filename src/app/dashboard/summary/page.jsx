...
function AdviceCard({ id, t, lang }) {
  ...
  const title =
    t[`advice_${numericId}_title`] ?? getAdviceItem(lang, numericId).title;
  const body =
    t[`advice_${numericId}_body`] ?? getAdviceItem(lang, numericId).body;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 11, color: MU, flexShrink: 0 }}>
          {open ? "▲" : "▼"}
        </span>
      </button>
      {open && (
        ...
        {relevantAdvice.length > 0 ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ fontSize: 11, color: MU, marginBottom: 4 }}>
              {relevantAdvice.length} {t.adviceBadgeRelevant ?? "relevant"} ·{" "}
              {t.adviceRelevantNote ?? "marked by patient"}
            </div>
          ...
          >
            ...
            <span style={{ fontSize: 12, color: MU }}>
              {t.noRelevantAdvice ??
                "Patient has not marked any advice as relevant yet."}
            </span>
          </div>
        )}
      </Card>
