      ...
      <div onClick={e=>e.stopPropagation()} style={{background:SU,borderRadius:20,width:"100%",maxWidth:500,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 28px 70px rgba(45,74,110,0.3)",border:`1px solid ${BO}`}}>
        ...
        <div style={{padding:18,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {[{label:"Mood",val:rec.mood,icon:"😊",max:5,color:"#4a7ab5"},{label:"Cravings",val:rec.cravings,icon:"🔥",max:5,color:"#f4a07a"},{label:"Wellbeing",val:rec.wellbeing,icon:"💙",max:5,color:"#9c27b0"}].map(s=>(
          ...
          {/* Frequency & Amount */}
          <Section title="Frequency & Amount">
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <Pill label="Frequency"    val={rec.frequency?(freqLabel[rec.frequency]??rec.frequency):"—"} color={A}/>
              <Pill label="Amount/Dose"  val={rec.amount!=null?rec.amount:"—"} color={AD}/>
            </div>
          </Section>
          {/* Substances */}
          ...
          <Section title="Weight">
            {rec.weight?<div style={{display:"flex",alignItems:"baseline",gap:5}}><span style={{fontSize:26,fontWeight:800,color:AD}}>{rec.weight}</span><span style={{fontSize:13,color:MU}}>kg</span></div>:<span style={{fontSize:12,color:MU}}>—</span>}
          </Section>
          {/* Note */}
          <Section title="Note">
...
export default function CalendarPage() {
  ...
  return(
    <div>
      ...
      <div className="cal-grid">
        ...
        <div style={{background:SU,borderRadius:14,border:`1px solid ${BO}`,boxShadow:"0 2px 10px rgba(74,122,181,0.07)",overflow:"hidden"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",padding:"0 10px",gap:2}}>
            {["M","T","W","T","F","S","S"].map((d,i)=><div key={i} style={{textAlign:"center",fontSize:9,fontWeight:700,color:MU,paddingBottom:3}}>{d}</div>)}
          </div>
          ...
          <div style={{borderTop:`1px solid ${BO}`,padding:"8px 14px",display:"flex",flexWrap:"wrap",gap:10}}>
            {[["#a8d5a2","None"],["#f5c97a","Low"],["#f4a07a","Moderate"],["#e87070","High"]].map(([c,l])=>(
              <div key={l} style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:8,height:8,borderRadius:2,background:c}}/><span style={{fontSize:9,color:MU}}>{l} cravings</span></div>
            ))}
          </div>
        </div>
        ...
        {/* Recent log */}
        <div>
          <div style={{fontSize:11,fontWeight:700,color:MU,letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Recent entries</div>
                ...
                <div key={i} onClick={()=>setModalDate(ds)} style={{background:SU,borderRadius:10,border:`1px solid ${BO}`,padding:"10px 12px",display:"flex",alignItems:"center",gap:10,cursor:"pointer",boxShadow:"0 1px 4px rgba(74,122,181,0.05)"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      ...
                      {!(r.substances??[]).length&&<span style={{fontSize:9,color:MU}}>No substances logged</span>}
                    </div>
                  </div>
      ...
      {modalDate&&recMap[modalDate]&&<DayModal date={modalDate} rec={recMap[modalDate]} onClose={()=>setModalDate(null)}/>}
    </div>
  );
}
