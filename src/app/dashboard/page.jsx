...
const QC = [
  ...
  { key: "latestReadiness", label: "Readiness to change", max: 30, color: "#0891B2",
...
function QuestionnaireModal({ qKey, data, onClose }) {
  ...
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(15,30,50,0.6)", backdropFilter: "blur(5px)",
        zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()}
        style={{ background: SU, borderRadius: 20, width: "100%", maxWidth: 500, maxHeight: "90vh",
          overflowY: "auto", boxShadow: "var(--shadow-elevated)", border: `1px solid ${BO}` }}>
        <div style={{ background: `linear-gradient(135deg,${def.color},${def.color}cc)`, borderRadius: "20px 20px 0 0",
          padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between",
          position: "sticky", top: 0, zIndex: 1 }}>
          <div>
            <div style={{ color: "rgba(255,255,255,0.7)", fontSize: 10, fontWeight: 700, letterSpacing: 1.2,
              textTransform: "uppercase", marginBottom: 2 }}>Questionnaire</div>
        ...
        <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ background: def.color + "10", border: `1px solid ${def.color}30`, borderRadius: 12,
            ...
            <div style={{ flex: 1 }}>
              <div style={{ height: 6, background: BG, borderRadius: 3, overflow: "hidden", marginBottom: 6 }}>
                <div style={{ width: `${pct}%`, height: "100%", background: def.color, borderRadius: 3,
            ...
            {def.questions.map(({ key, label }, qi) => {
              ...
              return (
                <div key={key} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 0",
                  ...
                  <div style={{ width: 26, height: 26, borderRadius: "50%", background: scoreColor, color: "#fff",
                    display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800,
                    flexShrink: 0 }}>{val}</div>
                </div>
              );
            })}
          </div>
          {rec.date && (<div style={{ fontSize: 10, color: MU, textAlign: "right" }}>Recorded: {rec.date}</div>)}
        </div>
      </div>
    </div>
  );
...
function Pill({ label, val, color }) {
  return (
    <div style={{ background: color + "18", border: `1px solid ${color}33`, borderRadius: 10,
    ...
    <div onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ background: `linear-gradient(135deg, var(--accent), var(--accent-strong))`,
          borderRadius: "20px 20px 0 0", padding: "16px 20px", display: "flex", alignItems: "center",
          justifyContent: "space-between", position: "sticky", top: 0, zIndex: 1 }}>
          <div>
            <div style={{ color: "rgba(255,255,255,0.65)", fontSize: 10, fontWeight: 700, letterSpacing: 1.2,
              textTransform: "uppercase", marginBottom: 2 }}>{t.dailyLog ?? "Daily log"}</div>
        ...
        <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
            {[
              ...
              <div key={s.label} style={{ background: BG, borderRadius: 12, padding: "11px 6px",
                textAlign: "center", border: `1px solid ${BO}` }}>
                ...
                <div style={{ height: 3, background: BO, borderRadius: 2, marginTop: 5, overflow: "hidden" }}>
                  <div style={{ width: s.val != null ? `${(s.val / s.max) * 100}%` : "0%", height: "100%",
          ...
          <Section title={t.frequency ?? "Frequency & Amount"}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <Pill label={t.frequency ?? "Frequency"}
                val={rec.frequency ? (freqLabel[rec.frequency] ?? rec.frequency) : "—"} color="#4a7ab5" />
          ...
          <Section title={t.substances ?? "Substances"}>
            {subs.length > 0 ? (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {subs.map((s) => (
                  <span key={s} style={{ background: sc(s) + "22", color: sc(s),
                    border: `1px solid ${sc(s)}44`, borderRadius: 20, padding: "5px 13px", fontSize: 12,
                    fontWeight: 600, textTransform: "capitalize" }}>{s}</span>
                ))}
              </div>
            ) : (
              <span style={{ background: "#22C55E22", color: "#16A34A", border: "1px solid #22C55E44",
                ...
                {meds.map((med, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: BG, borderRadius: 10, padding: "10px 12px",
                      ...
                      {dose != null && (
                        <span style={{ fontSize: 10, background: AL, color: AD, borderRadius: 20,
                          padding: "2px 8px", fontWeight: 600 }}>{dose}</span>
                      )}
                    </div>
                  );
                })}
          ...
          <Section title={t.sideEffects ?? "Side effects"}>
            {effects.length > 0 ? (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {effects.map((e) => (
                  <span key={e} style={{ background: "var(--warn-soft)", color: "var(--warn)",
                    border: "1px solid var(--warn-soft)", borderRadius: 20, padding: "5px 13px", fontSize: 12 }}>{e}</span>
                ))}
              </div>
            ) : (<span style={{ fontSize: 12, color: MU }}>—</span>)}
          </Section>
          ...
          <Section title={t.note ?? "Note"}>
            {rec.note ? (
              <div style={{ background: AL, borderRadius: 10, padding: "12px 14px", fontSize: 13, color: TX,
                borderLeft: `3px solid var(--accent)`, fontStyle: "italic", lineHeight: 1.7 }}>
                "{rec.note}"
              </div>
            ) : (<span style={{ fontSize: 12, color: MU }}>—</span>)}
          </Section>
...
export default function CalendarPage() {
  ...
  const months = t.months ?? ["January","February","March","April","May","June",
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", width: "100%" }}>
      ...
      <div className="cal-grid">
        {/* ── Calendar card ── */}
        <div style={{ background: SU, borderRadius: 14, border: `1px solid ${BO}`,
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
            ...
            <button onClick={() => setMonth((p) => {
              ...
              style={{ background: "none", border: `1px solid ${BO}`, borderRadius: 6, width: 26, height: 26,
                cursor: "pointer", color: MU, fontSize: 13, display: "flex", alignItems: "center",
                justifyContent: "center", fontFamily: "inherit" }}>‹</button>
            <span style={{ fontSize: 11, fontWeight: 700, color: A, letterSpacing: 1.2,
              textTransform: "uppercase" }}>{months[m]} {y}</span>
            <button onClick={() => setMonth((p) => {
                const d = new Date(p.y, p.m + 1);
                return { y: d.getFullYear(), m: d.getMonth() };
              })}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", padding: "0 10px", gap: 2 }}>
            {weekdays.map((d, i) => (
              <div key={i} style={{ textAlign: "center", fontSize: 9, fontWeight: 700, color: MU,
                paddingBottom: 3 }}>{d}</div>
            ))}
          </div>
          {/* Day cells */}
            ...
            {Array.from({ length: days }).map((_, i) => {
              ...
              return (
                <div key={day} onClick={() => rec && setModalDate(ds)}
                  style={{ borderRadius: 6, padding: "4px 1px", textAlign: "center",
                    cursor: rec ? "pointer" : "default", minHeight: 30, background: dayBg(rec),
                  ...
                  <div style={{ fontSize: 10, fontWeight: isToday ? 700 : 400,
                    color: rec ? (dayScore(rec) >= 3.5 ? "#fff" : "#1a2c3d") : MU, lineHeight: 1 }}>
                    {day}
                  </div>
                  {sober && (
                    <svg viewBox="0 0 14 14" width="13" height="13" role="img" aria-label={soberLabel}
                      style={{ position: "absolute", top: -4, left: -4, pointerEvents: "none" }}>
                      <title>{soberLabel}</title>
                      <path d="M 7 0 L 8.5 5.5 L 14 7 L 8.5 8.5 L 7 14 L 5.5 8.5 L 0 7 L 5.5 5.5 Z"
                        fill="#d4a017" stroke="#8a6a0e" strokeWidth="0.6" strokeLinejoin="round" />
                    </svg>
                  )}
                  {tookMeds && (
                    <img src="/ico_medicine.png" alt="medication taken"
                      style={{ position: "absolute", top: -4, right: -4, width: 13, height: 13,
                        objectFit: "contain" }} />
                  )}
                  {highCravings && (
                  ...
                  {hasNote && (
                    <svg viewBox="0 0 20 20" style={{ position: "absolute", bottom: -4, right: -4,
                      width: 13, height: 13 }} fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Indicator legend */}
          <div style={{ padding: "0 14px 10px", display: "flex", flexWrap: "wrap",
            alignItems: "center", gap: "4px 12px", fontSize: 10, color: MU }}>
            {/* Sober — top-left */}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ fontSize: 11, lineHeight: 1 }} aria-hidden="true">🔥</span>
          ...
          <div style={{ borderTop: `1px solid ${BO}`, padding: "10px 14px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: A, letterSpacing: 1.2,
                  ...
                  return monthSubCounts.map(([s, n]) => (
                    <div key={s}>
                      ...
                      <div style={{ height: 5, background: BG, borderRadius: 3, overflow: "hidden" }}>
                        <div style={{ width: `${(n / max) * 100}%`, height: "100%", background: sc(s),
          ...
          <div style={{ borderTop: `1px solid ${BO}`, padding: "10px 14px" }}>
            ...
            {profileMeds.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: MU, letterSpacing: 0.8,
                  textTransform: "uppercase", marginBottom: 6 }}>{t.prescribed ?? "Prescribed"}</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {profileMeds.map((name) => (
                    <span key={name} style={{ background: AL, color: AD, border: `1px solid ${BO}`,
            ...
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: MU, letterSpacing: 0.8,
                textTransform: "uppercase", marginBottom: 6 }}>{t.takenThisMonth ?? "Taken this month"}</div>
                ...
                (() => {
                  ...
                  return monthMedCounts.map(([name, n]) => (
                    <div key={name} style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ height: 5, background: BG, borderRadius: 3, overflow: "hidden" }}>
                        <div style={{ width: `${(n / max) * 100}%`, height: "100%", background: "#42a5f5",
                          borderRadius: 3, transition: "width .4s ease" }} />
                      </div>
                    </div>
                  ));
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ background: SU, borderRadius: 14, border: `1px solid ${BO}`,
            boxShadow: "var(--shadow-card)", overflow: "hidden", flex: 1 }}>
            {/* Monthly Averages */}
            <div style={{ padding: "12px 14px 10px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12,
                padding: "6px 10px", background: AL, borderRadius: 8 }}>
                ...
                return (
                  <div key={q.key} onClick={() => data[q.key] && setQModal(q.key)}
                    ...
                    style={{ padding: "9px 6px",
                      ...
                      cursor: data[q.key] ? "pointer" : "default", borderRadius: 6,
                      ...
                      <>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ flex: 1, height: 4, background: BG, borderRadius: 2, overflow: "hidden" }}>
                            <div style={{ width: `${Math.min(100, (total / q.max) * 100)}%`, height: "100%",
                              background: q.color, borderRadius: 2 }} />
                          </div>
                          <span style={{ fontSize: 10, color: q.color, fontWeight: 700, flexShrink: 0,
